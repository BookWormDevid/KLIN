"""
VideoMAE binary clip-level training script (NUM_FRAMES = 16)

Requirements:
    pip install torch torchvision transformers accelerate scikit-learn mlflow opencv-python
...
MODEL_NAME = str(BASE_DIR / "models" / "videomae-large")  # or HF hub id
...
class WeightedTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        # Weighted loss
        weights = torch.tensor(CLASS_WEIGHTS, dtype=torch.float32).to(
            torch.device(logits.device)
        )
...
def main() -> None:
    ...
    train_dataset = BetterVideoDataset(TRAIN_DIR, mode="train")
    val_dataset = BetterVideoDataset(VAL_DIR, mode="val")
    test_dataset = BetterVideoDataset(TEST_DIR, mode="test")
    ...
    print("Running multi-clip evaluation on validation set...")
    mc_metrics = multi_clip_evaluate(trainer, val_dataset, n_clips=5)
    print("Multi-clip validation metrics:", mc_metrics)
    mlflow.log_metrics({f"multiclip_val_{k}": v for k, v in mc_metrics.items()})

    print("Running test evaluation (single-clip)...")
