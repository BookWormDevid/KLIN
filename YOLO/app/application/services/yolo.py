...
from YOLO.app.application.dto import YoloProcessDto, YoloReadDto, YoloUploadDto
from YOLO.app.application.interfaces import (
    IYoloInference,
    IYoloProcessProducer,
    IYoloRepository,
)
from YOLO.app.models import ProcessingState, YoloModel
...
class YoloService:
    ...
    async def get_inference_status(self, yolo_id: uuid.UUID) -> YoloReadDto:
        yolo = await self._yolo_repository.get_by_id(yolo_id)
        if not yolo:
            raise ValueError(f"yolo {yolo_id} not found")
        return YoloReadDto.from_model(yolo)
