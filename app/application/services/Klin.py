...
class KlinService:
    ...
    async def klin_image(
        self, data: KlinUploadDto
    ) -> KlinModel:
        klin = KlinModel(
            target_url=data.target_url, state=ProcessingState.PENDING,
        )
        klin = await self._klin_repository.create(klin)

        await self._klin_process_producer.send(
            KlinProcessDto(klin_id=klin.id)
        )
